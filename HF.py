def add_more_seq_scan(model,tokenizer,temp_unfilt_ans,temp_questions,old_pr,old_ans,device,max_tries=7,increase_threshold=5,verbose=True):
    i=0
    final_ans = old_ans
...
def answer_batch_questions(model,tokenizer,model_questions,pr,label,cache_fn,max_len,device,verbose=False):
    ...
    for i in range(ceil(len(pr)/10)):
        batch_pr = pr[i*10:i*10+10]   #[i:i+10]
        batch_mq = model_questions[i*10:i*10+10]   #[i:i+10]
        ...
        if to_fetch_indices:
            ...
            #print('batch_pr_to_fetch:',batch_pr_to_fetch)
            #print('batch_pr',batch_pr)
            #print('INDICES TO BE FETCHED',to_fetch_indices)   
            ...
            batch_ans_fetched = ans # [response['choices'][j]['text'] for j in range(len(batch_pr_to_fetch))]
...
def compute_node(model,tokenizer,node,instr,few_shots,inst_funct,label,augmented_question_maps,device,verbose=False):
    ...
    if node.op=='JOIN':
        ...
        print(len(lpr),len(node.l.answers[-1]),len(batch_left_ans))
...
def compute_tree(model,tokenizer,node,instr,few_shots,inst_funct,label,augmented_question_maps,device,verbose=False):
...
def HF_SPW_seq(model,tokenizer,df,instr,few_shots,inst_funct,label,augmented_question_maps,query_plan_dict,device,verbose=False):
    global b
